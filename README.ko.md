...
- 알림 클릭으로 작업 창 복귀 - 토스트를 누르면 알림이 발생한 Codex 데스크톱 또는 VS Code 창을 전면으로 가져옵니다.
- 창이 없어도 프로젝트 열기 - 대상 VS Code 창이 닫혔으면 `code -n <project>` 방식으로 프로젝트 창을 다시 엽니다.
- 포커스 인식 - 이미 대상 창을 보고 있으면 알림을 생략할 수 있고, 다시 포커스되면 해당 창의 알림만 정리합니다.
...
> Job-Finish는 Windows 전용 도구입니다. PowerShell과 Windows toast API, Codex 데스크톱/VS Code 창 포커스 처리를 사용합니다.
...
| 포커스 억제 | 이미 대상 앱 창을 보고 있을 때 알림 생략 여부 |
...
또는 Codex notify
  ...
  -> jf-focus-vscode.exe가 알림이 발생한 Codex 데스크톱 또는 VS Code 창 탐색
  -> 정확한 창을 전면으로 가져오거나 프로젝트를 새 창으로 열기
```

Job-Finish는 단순히 알림만 띄우지 않습니다. Codex 데스크톱 이벤트는 작업이 시작된 데스크톱 프로세스를 따라가고, VS Code 이벤트는 프로젝트명, cwd, window handle, process id를 활용해 가장 적합한 창을 찾습니다. 알림 클릭과 작업표시줄 깜빡임이 같은 창을 가리키므로 여러 프로젝트를 동시에 작업할 때도 헷갈리지 않습니다.
