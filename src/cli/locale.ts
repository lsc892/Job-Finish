...
const COPIES: Record<InstallLocale, InstallCopy> = {
  en: {
    ...
    init: {
      ...
      codexGlobalWarning: "Codex has only one global notify setting, so a project install can also change the global Codex setting.",
      keepGlobalTitle: "Keeping global install",
      globalOnly: "Only the global install remains.",
      globalInstall: "global install",
      globalCodexNotify: "global Codex notify",
      ...
      projectCodexRemoved: "removed Codex notify that pointed to the project install",
      projectInstallRemoved: "removed project install folder",
      reset: "Reset",
      previousResidueRemoved: "removed previous install residue",
      backup: "backup ✓",
      codexConflict: "Codex: existing notify setting found; skipped — manual setup required",
      codexOnlyOne: "Codex supports only one notify command. The existing setting was kept.",
      addManually: "To add it manually, add this to",
  ...
  ko: {
    ...
    init: {
      ...
      codexGlobalWarning: "Codex notify는 전역 설정이 하나뿐이라 프로젝트 설치도 전역 Codex 설정을 바꿀 수 있어요.",
      keepGlobalTitle: "전역 설치 유지",
      globalOnly: "전역 설치만 남도록 처리했어요.",
      globalInstall: "전역 설치",
      globalCodexNotify: "전역 Codex notify",
      ...
      projectCodexRemoved: "프로젝트 설치본을 가리키던 Codex notify 제거",
      projectInstallRemoved: "프로젝트 설치 폴더 삭제",
      reset: "리셋",
      previousResidueRemoved: "이전 설치 잔여 정리",
      backup: "백업 ✓",
      codexConflict: "Codex: 기존 notify 설정이 있어 건너뜀 — 수동 설정 필요",
      codexOnlyOne: "Codex의 notify는 하나만 가능해요. 기존 설정을 유지했습니다.",
      addManually: "직접 추가하려면 다음 파일에 입력하세요",
  ...
  zh: {
    ...
    init: {
      ...
      codexGlobalWarning: "Codex 只有一项全局 notify 设置，因此项目安装也可能更改全局 Codex 设置。",
      keepGlobalTitle: "保留全局安装",
      globalOnly: "已处理为仅保留全局安装。",
      globalInstall: "全局安装",
      globalCodexNotify: "全局 Codex notify",
      ...
      projectCodexRemoved: "已移除指向项目安装的 Codex notify",
      projectInstallRemoved: "已删除项目安装文件夹",
      reset: "重置",
      previousResidueRemoved: "已清理之前的安装残留",
      backup: "已备份 ✓",
      codexConflict: "Codex：已存在 notify 设置，已跳过 — 需要手动配置",
      codexOnlyOne: "Codex 仅支持一项 notify 命令，已保留现有设置。",
      addManually: "如需手动添加，请写入",
  ...
  jp: {
    ...
    init: {
      ...
      codexGlobalWarning: "Codexのnotify設定はグローバルに1つだけなので、プロジェクトへのインストールでもグローバル設定が変更される場合があります。",
      keepGlobalTitle: "グローバルインストールを維持",
      globalOnly: "グローバルインストールだけを残しました。",
      globalInstall: "グローバルインストール",
      globalCodexNotify: "グローバルCodex notify",
      ...
      projectCodexRemoved: "プロジェクトのインストールを指していたCodex notifyを削除",
      projectInstallRemoved: "プロジェクトのインストールフォルダを削除",
      reset: "リセット",
      previousResidueRemoved: "以前のインストールの残骸を削除",
      backup: "バックアップ ✓",
      codexConflict: "Codex：既存のnotify設定があるためスキップ — 手動設定が必要です",
      codexOnlyOne: "Codexのnotifyコマンドは1つだけ設定できます。既存の設定を維持しました。",
      addManually: "手動で追加する場合は、次のファイルに記述してください",
